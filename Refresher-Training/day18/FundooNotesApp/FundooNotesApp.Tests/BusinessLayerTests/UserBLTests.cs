...
{
    private static (UserBL userBL, FakeUserRepository repo, FakeEmailQueuePublisher emailQueue, FakeResetTokenCache resetTokenCache) CreateSystemUnderTest()
    {
        ...
        // Fakes so tests don't need a real RabbitMQ or Redis server.
        var emailQueue = new FakeEmailQueuePublisher();
        var resetTokenCache = new FakeResetTokenCache();

        var userBL = new UserBL(repo, tokenGenerator, emailQueue, resetTokenCache);

        return (userBL, repo, emailQueue, resetTokenCache);
    }

    [TestMethod]
    public void Register_ShouldThrow_WhenEmailAlreadyExists()
    {
        var (userBL, repo, _, _) = CreateSystemUnderTest();
    ...
    {
        var (userBL, _, _, _) = CreateSystemUnderTest();
