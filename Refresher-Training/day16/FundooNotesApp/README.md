...
The Business layer for notes is deliberately split into **two**
classes instead of one:

- **`NoteCommandBL`** - only handles operations that CHANGE data:
  create, pin, archive, trash, restore, delete. All the business
  rules (like "pin and archive can't both be true") live here.
- **`NoteQueryBL`** - only handles operations that READ data: get all,
  get by id, search, filter, and a summary/dashboard count. Nothing
  in this class is ever allowed to modify the database.

The Controller depends on both interfaces (`INoteCommandBL` and
`INoteQueryBL`) and simply calls whichever one matches the HTTP verb -
`GET` requests go to the query side, everything else (`POST`, `PATCH`,
`DELETE`) goes to the command side. Both sides still share the same
`INoteRL` repository underneath - splitting the repository too would
be the "full" version of CQRS (often with separate read/write
databases), but for a project this size, splitting just the Business
layer already demonstrates the core idea cleanly.

### Advanced LINQ examples

- **`GetAllNotes`** chains `OrderByDescending` + `ThenByDescending` to
  show pinned notes first, then newest-first within each group.
- **`GetNotesSummary`** uses `GroupBy` to bucket every note into
  Active / Pinned / Archived / Trashed and count each group in a
...
BusinessLayer/Interfaces/INoteCommandBL.cs      <- CQRS: writes
BusinessLayer/Interfaces/INoteQueryBL.cs        <- CQRS: reads
BusinessLayer/Services/NoteCommandBL.cs
BusinessLayer/Services/NoteQueryBL.cs
BusinessLayer/Services/NoteMapper.cs            <- shared Entity->Model helper
...
`NoteCommandBL` no longer knows or cares who reacts when a note is
created or trashed - it just "announces" the event through
`INoteEventPublisher`. A separate `NoteActivityLogger` class listens
for those announcements and writes them to the log, completely
decoupled from the note-creation code itself.

```
NoteCommandBL.CreateNote()
        |
        v
 _eventPublisher.PublishNoteCreated(...)   <- publisher has no idea who's listening
        |
        v
 NoteActivityLogger.HandleNoteCreated(...)  <- subscribed once, in Program.cs
        |
        v
   writes an [Activity] line via NLog
```

**New files:** `BusinessLayer/Events/NoteCreatedEvent.cs`,
`BusinessLayer/Events/NoteTrashedEvent.cs`,
`BusinessLayer/Events/NoteEventPublisher.cs` (the pub-sub hub),
`BusinessLayer/Events/NoteActivityLogger.cs` (the subscriber).

## Logging with NLog

Every layer can now inject `ILogger<T>` (see `UserBL.cs` and
`TagBL.cs` for examples) to write structured log messages. NLog reads
`FundooNotesApp.API/nlog.config` to decide where those messages go:

- A rolling file: `logs/fundoonotes-<date>.log`
- The console, while you're running `dotnet run`

Try it: register a user, then check the `logs/` folder inside
`FundooNotesApp.API/bin/Debug/net8.0/` - you'll see a line like
`New user registered: test@example.com`.

## Unit Testing with MSTest

A new `FundooNotesApp.Tests` project holds unit tests for the
Business layer - `NoteCommandBLTests`, `UserBLTests`, and
`TagBLTests`. Instead of a mocking library, these tests use small
hand-written "fake" repositories (`FundooNotesApp.Tests/Fakes/`) that
store data in a plain in-memory list - fast, no database needed, and
