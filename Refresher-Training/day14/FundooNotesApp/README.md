...
## Notes Management Module (Day 14)

Every endpoint under `/api/note/...` requires you to be logged in -
the whole `NoteController` is marked `[Authorize]`. Notes are always
linked to whoever's token you used; there is no way to create or
delete a note "as" someone else.

| Method | URL                         | What it does                               |
|--------|-------------------------------|-----------------------------------------------|
| POST   | `/api/note/create`            | Create a note owned by the logged-in user      |
| DELETE | `/api/note/delete/{noteId}`   | Delete a note - only if it belongs to you      |

### Create a note (body)
```json
{ "title": "Grocery List", "content": "Milk, eggs, bread" }
```
Response includes the new `noteId` you'll need for deleting it later.

### Delete a note
Just call `DELETE /api/note/delete/5` (replace `5` with your note's id) -
no request body needed. If that id doesn't exist, or belongs to a
different user, you'll get a 404 - the API deliberately doesn't reveal
which of the two happened, to avoid leaking other users' note ids.

**New folder additions for this module:**
```
...
Since we added a new `Notes` table, you'll need one more migration
before this works (see "How to run it" below - just repeat the
migration commands with a new name, e.g. `AddNotesTable`).
